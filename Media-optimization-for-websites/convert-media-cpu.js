...
function discoverFiles(sourceDir, patterns) {
  const files = [];
...
async function optimizeImage(inputPath, outputDir, config) {
  try {
    ...
    const relativeDir = path.dirname(inputPath).split('public')[1] || '';
    const webpOutput = path.join(
      outputDir,
      relativeDir,
      `${filename}.webp`
    );
    const pngOutput = path.join(
      outputDir,
      relativeDir,
      `${filename}.png`
    );
    ...
    if (isAnimated) {
      // For animated GIFs, convert to WebM video (handled separately)
      return {
        status: 'skipped',
        reason: 'Animated GIF - convert to video',
        originalSize: fileSize,
        optimizedSize: fileSize
      };
    }
    ...
    const quality = sourceExt === '.png' ? undefined : config.imageQuality;
    ...
    optimizedSize = fs.statSync(webpOutput).size;

    // Also create PNG fallback for important images
...
function optimizeVideo(inputPath, outputDir, config, isMobile = false) {
  return new Promise((resolve) => {
    try {
      ...
      const filename = path.basename(inputPath, path.extname(inputPath));
      const relativeDir = path.dirname(inputPath).split('public')[1] || '';
      const targetDir = isMobile
        ? path.join(outputDir, 'mobile', relativeDir)
        : path.join(outputDir, relativeDir);

      fs.mkdirSync(targetDir, { recursive: true });

      const mp4Output = path.join(targetDir, `${filename}.mp4`);
      const webmOutput = path.join(targetDir, `${filename}.webm`);

      let completedFormats = 0;
      let totalFormats = 2;
      const results = { mp4: null, webm: null };
      ...
      ffmpeg.ffprobe(inputPath, (err, metadata) => {
        ...
        let width = metadata.streams[0]?.width || 1920;
        let height = metadata.streams[0]?.height || 1080;
        ...
        ffmpeg(inputPath)
          .outputOptions([
            ...
            '-preset medium',
            `-vf scale=w=${width}:h=trunc(ow/a/2)*2`, // Ensure divisible by 2
            '-c:a aac',
            '-b:a 128k'
          ])
          .output(mp4Output)
        ...
        ffmpeg(inputPath)
          ...
          .on('end', () => {
            results.webm = {
              path: webmOutput,
              size: fs.statSync(webmOutput).size
            };
            completedFormats++;
            if (completedFormats === totalFormats) {
              finalize();
...
async function main() {
  ...
  const argv = await yargs(hideBin(process.argv))
    ...
    .option('no-backup', {
      describe: "Don't keep original files in output",
      type: 'boolean',
      default: false
    })
    .option('verbose', {
  ...
  console.log(chalk.cyan.bold('║   💻 CPU MODE (libx264 / libvpx-vp9)   ║'));
  console.log(chalk.cyan.bold('╚════════════════════════════════════════╝\n'));

  logger.info('Starting CPU-mode media optimization', {
    sourceDir,
    outputDir,
    dryRun: argv['dry-run'],
